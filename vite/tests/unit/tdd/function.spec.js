...
describe('插件功能测试', () => {
  ...
  it('verify开关测试', async () => {
    ...
    const FormWrapper = WatchWrapper.findComponent({ ref: 'form' })
    const SubmitBut = WatchWrapper.find('.watch-component-but__submit')

    // 设置参数
    FormWrapper.setProps({ form__: { salePrice: 50, originPrice: 40 } })

    // 2. 空内容校验 - 期望：必填项，校验不通过
    await SubmitBut.trigger('click')
    eventCollecte = FormWrapper.emitted('validate')
    console.log(eventCollecte)
  });
});
